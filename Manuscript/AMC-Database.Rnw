...
For an empirical analysis of the word/sentence count and raw motive scores relations, we reduced the dataset to \Sexpr{big(nrow(person.gt50.each))} persons, nested in \Sexpr{length(unique(person.gt50.each$study_id))} studies which did not apply the 2nd-sentence rule and did not use pictures with too few stories. 
...
Based on the $k$ = \Sexpr{MA.resid.aff$k} primary studies in the database which provide the subject's gender and do have variation in gender we (a) aimed to replicate the reported gender differences (female minus male), and (b) investigated the impact of the different ways of controlling for story length.
