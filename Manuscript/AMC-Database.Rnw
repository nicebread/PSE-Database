...
load("../R/cache/1-Descriptives.RData")
load("../R/data/tab.codebook.RData")
load("../R/cache/MLMs.RData")
load("../R/cache/CAs.RData")
load("../R/cache/picture_norms.RData")
load("../R/cache/tab.sent.RData")
load("../R/cache/person.RData")
load("../R/cache/Decline.RData")
source("../R/0-helpers.R")
@

We present two openly accessible databases related to the assessment of implicit motives using Picture Story Exercises (PSEs): (a) A database of \Sexpr{big(nrow(PSE))} German sentences, nested in \Sexpr{big(nrow(story.desc))} stories provided by \Sexpr{big(length(unique(PSE$person_ID)))} participants, which have been coded by experts using Winter's (1994) coding system for the implicit affiliation/intimacy, achievement, and power motives, and (b) a database of \Sexpr{length(unique(PSE$pic_ID))} classic and new pictures which have been used as PSE stimuli. Updated picture norms are provided which can be used to select appropriate pictures for PSE applications.
...
The current paper has three goals: (1) To present a large database of stories that have been coded for implicit motives using the Winter coding system, (2) to provide a systematic database of \Sexpr{length(unique(PSE$pic_ID))} classic and new picture stimuli that have been used in PSEs, and (3) to provide updated norms for picture pulls (i.e., the propensity of a picture to elicit a certain kind of motive image).
...
The database combines coded PSE stories from \Sexpr{length(unique(PSE$study_ID))} studies. Overall, \Sexpr{length(unique(PSE$pic_ID))} different pictures were used, although \Sexpr{sum(grepl("newpic", unique(PSE$pic_ID)))} of them (``newpic'') were just recently added and have only very few coded stories. Only \Sexpr{table(story.desc$newpic)["TRUE"]} (=\Sexpr{f2(prop.table(table(story.desc$newpic))["TRUE"]*100, 1)}\%) of all stories were written in response to these new pictures. Therefore all picture-related descriptive statistics below have been computed for the remaining pictures, each of which had at least \Sexpr{min(t1dat$n.stories)} stories.

Overall, the database consists of \Sexpr{big(nrow(PSE))} sentences coded with the Winter system, which are nested in \Sexpr{big(nrow(story.desc))} stories provided by \Sexpr{big(length(unique(PSE$person_ID)))} participants. Most participants wrote stories to 5 pictures (\Sexpr{f2(prop.table(table(person.desc$n.stories))["5"]*100, 1)}\%) or 6 pictures (\Sexpr{f2(prop.table(table(person.desc$n.stories))["6"]*100, 1)}\%) during their PSE task. The other studies had four, seven, or eight pictures. A story had on average \Sexpr{f2(mean(story.desc$sc.story), 1)} (\emph{SD} = \Sexpr{f2(sd(story.desc.noNewpic$sc.story), 1)}) sentences and \Sexpr{f2(mean(story.desc$wc.story), 1)} (\emph{SD} = \Sexpr{f2(sd(story.desc.noNewpic$wc.story), 1)}) words. These counts were roughly comparable for all pictures, ranging from an average sentence count of \Sexpr{f2(sentenceCountByPic$sc.story.mean[1], 1)} for picture \emph{\Sexpr{sanitize(sentenceCountByPic$pic_ID[1])}} to \Sexpr{f2(sentenceCountByPic$sc.story.mean[nrow(sentenceCountByPic)], 1)} for \emph{\Sexpr{sentenceCountByPic$pic_ID[nrow(sentenceCountByPic)]}}. The average word count was between \Sexpr{f2(min(sentenceCountByPic[-1, "wc.story.mean"]))} and \Sexpr{f2(max(sentenceCountByPic[-1, "wc.story.mean"]))}, except for picture \emph{\Sexpr{sanitize(sentenceCountByPic$pic_ID[1])}} which had only \Sexpr{f2(sentenceCountByPic[1, "wc.story.mean"])} words on average.
...
@ 
		...
		\begin{tablenotes}[para,flushleft]
			{\small
			\vspace*{0.75em}
			\textit{Note.} Analyses in this table are based on \Sexpr{nrow(person)} persons, nested in \Sexpr{length(unique(person$study_ID))} studies which did not apply the 2nd-sentence rule and did not use the new pictures. Mean and \emph{SD} are computed per picture story, as the number of pictures varies between studies. The correlations are computed within study on person level and then meta-analytically aggregated across studies.}
	      \end{tablenotes}
	  \end{threeparttable}
%\end{table*}
\end{sidewaystable*}
...
\multirow{3}{*}{\makecell[l]{Commonality analysis: How much of the\\ explained variance can be attributed\\to unique and common parts of predictors?}}   & Common to $sc + wc$ & \Sexpr{f2(CA.aff$common.R2.perc*100, 1)}\% & \Sexpr{f2(CA.ach$common.R2.perc*100, 1)}\% & \Sexpr{f2(CA.pow$common.R2.perc*100, 1)}\%\\
...
It is a well established finding that, after word count correction, women have higher motive scores in the implicit affiliation motive than men (see the meta-analysis by XXX). 
For the power and the achievement motive, in contrast, the same meta-analysis did not find significant gender differences.

Based on the $k$ = \Sexpr{MA.resid$k} primary studies in the database which provide the subject's gender and do have variation in gender we (a) tried to replicate the reported gender differences, and (b) investigated the impact of the different ways of controlling for story length.

We ran a random effects meta-analysis using the \emph{metafor} package \parencite{viechtbauer_conducting_2010}, with Hedge's $g$ as effect size measure. Table~\ref{tab:MA} reports the results.
