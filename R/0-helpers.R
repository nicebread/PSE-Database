...
# if a number is, say -0.0001, trim it to exactly zero 
# (otherwise, it is displayed as -.00 in the correlation matrix)
...
# print a number in bold if it exceeds "bold.cutoff"
f2bold <- function(x, digits=0, bold.cutoff=1) {
	paste0(ifelse(x>=bold.cutoff, "\\textbf{", ""), f2(x, digits), ifelse(x>=bold.cutoff, "}", ""))
}

# print a number x in bold if another variable y exceeds "bold.cutoff"
