...
person$aff.dens <- person$aff.sum/(person$wc.person/100)
person$ach.dens <- person$ach.sum/(person$wc.person/100)
person$pow.dens <- person$pow.sum/(person$wc.person/100)
...
res.aff <- data.frame(res.aff, ES)
res.ach <- data.frame(res.ach, ES)
res.pow <- data.frame(res.pow, ES)

# get meta-analytic effect size for each correction method
library(metafor)
(MA.resid.aff <- rma(yi=res.aff %>% filter(correction=="resid") %>% pull("yi"), vi=res.aff %>% filter(correction=="resid") %>% pull("vi")))
(MA.robust.aff <- rma(yi=res.aff %>% filter(correction=="robust") %>% pull("yi"), vi=res.aff %>% filter(correction=="robust") %>% pull("vi")))
(MA.density.aff <- rma(yi=res.aff %>% filter(correction=="density") %>% pull("yi"), vi=res.aff %>% filter(correction=="density") %>% pull("vi")))
...
MA_to_string <- function(obj, het=FALSE) {
	res <- paste0("$g = ", f2(obj$b, 2),"$ (\\emph{SE} = ", f2(obj$se, 2), ", $p$ = ", p0(obj$pval, latex=TRUE))
	if (het==FALSE) {
		res <- paste0(res, ")")
	} else {
		res <- paste0(res, "; Q(", obj$k-1,") = ", f2(obj$QE, 2), ", $p$ = ", f2(obj$QEp, 3, skipZero=TRUE), "; $I^2$ = ", round(obj$I2),"\\%)")
	}	
	return(res)
}

MA_to_string(MA.density)

data.frame(
	correction = c("Density scores", "OLS residuals", "Robust residuals"),
	aff.MA = c(MA_to_string(MA.density), MA_to_string(MA.resid), MA_to_string(MA.robust))
